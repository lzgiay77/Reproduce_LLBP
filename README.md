...
（2）操作系统：使用 Ubuntu 22.04.1。

（3）编译器：需要支持 C++17 的编译器（如 g++-9 或更高版本）
...
（1）调整llbp.h文件中的accessDelay = 6；

（2）调整llbp.cc文件中HASHVALS的忽略距离 D = 4；

（3）热身阶段 (Warm-up)：100M (1亿) 条指令 。
...
```
（2）其他的traces命令只需要替换相应的traces数据集的名称和记录结果的txt文件名称即可。
